...
class Push2Proxy(object):
    ...
    def __init__(self, c_instance, host, io):
        ...
        try:
            ...
            self._song_observer.attach(song_view)
        except Exception:
        ...
        try:
            ...
            if RedringObserver is not None:
                self._redring = RedringObserver.start(self._module_manager, self._io)
                self._redring.redraw_frame(True)
            else:
                _io.send("debug", "redring: observer missing (skipped)")
        except Exception:
