...
# Purpose
# -------
# Provide lazy, static (per Live version) data into the shared cache:
#   - banks
#   - colors
#
# Contract
...
#       _push2_state_data['banks']  = StaticDataObserver._load_banks_normalized
#       _push2_state_data['colors'] = StaticDataObserver._load_colors_normalized
# - The first time a callable is invoked it:
#       1) loads raw static data from Push2Original.*
#       2) normalizes it (json-safe, stable shapes)
#       3) returns the normalized data (Data._emit_path replaces callable with data)
# - No reporting/emit is done here. Output routing is handled elsewhere.
...
class StaticDataObserver(object):
    ...
    # Widgets: midi codes and theair names
    # Conversion tabel for MIDI id's to button and encodert names
    # A value < 128 means it is sent as a CC message
    # A value >= 128 means it is sent as a MIDI Note. 128 is added to distinguish it from a MIDI CC
